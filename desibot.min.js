(function loadDesiBotCSS() {
    ...
    //link.href = "./desibot.min.css";
     link.href = "https://cdn.jsdelivr.net/gh/darksmogai/desiaibot@v1.0.5/desibot.min.css"; // ✅ replace with your actual CDN CSS URL
...
window.initDesiBot = function ({
  ...
  <div class="desibot-container">
    ...
    <div class="desibot-meta">
      ${sender === "user" ? "✔✔" : ""} ${formatTime(new Date())}
    </div>
  `;
...
function formatTime(date) {
        ...
        fetch(`${backendUrl}/api/chat/history`, {
          ...
          .then(data => {
            if (data && Array.isArray(data)) {
              ...
              // 👋 Welcome back message after loading history
              if(data.length>0){
              setTimeout(() => {
                appendMessage(`👋 Welcome back, ${userName}!`, "bot");
              }, 300); // small delay to ensure it shows after history
            }
        }
          })
          .catch(() => appendMessage("⚠️ Unable to fetch chat history."));
      }
    ...
    function handleInput() {
      ...
      if (step === 0) {
        if (/^\d{10}$/.test(text)) {
          ...
          appendMessage("⚠️ Please enter a valid 10-digit mobile number.", "bot");
          input.value = "";
        }
      } else if (step === 1) {
        userName = text;
  ...
  fetchChatHistory();
        ...
        fetch(`${backendUrl}/api/chat/send`, {
          ...
          .then(res => res.json())
          .then(data => appendMessage(data.message || "✔️ Received"))
          .catch(() => appendMessage("⚠️ Unable to reach server."));
      }
    }
    ...
    if (isFirstTime) {
      appendMessage("👋 Hey! I'm DesiBot. Please enter your mobile number.", "bot");
