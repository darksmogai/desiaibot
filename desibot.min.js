(function loadDesiBotCSS() {
  ...
  link.rel = "stylesheet";
  link.href = "http://darksmogai.com/lib/desibot.min.css";
...
window.initDesiBot = function ({
  ...
  window.addEventListener("load", () => {
    ...
    //launcher.innerHTML = "🤖";
    launcher.innerHTML = `<img src="https://cdn.jsdelivr.net/gh/darksmogai/desiaibot@v1.0.7/bot-logo.png" alt="DesiBot" style="width: 30px; height: 30px;" />`;
    ...
    <svg viewBox="0 0 500 80" preserveAspectRatio="none"
      ...
      <path d="M0,30 C150,90 350,-30 500,30 L500,80 L0,80 Z" fill="#f0f4f8"></path>
          ...
          <div id="desibot-input-container">
            <input type="text" id="desibot-input" autocomplete="off" placeholder="Enter your mobile number..." maxlength="10" />
            <button id="desibot-send">➤</button>
          </div>
  ...
  <a href="https://darksmogai.com" target="_blank"  style="text-decoration: none; color: inherit;" rel="noopener noreferrer">
    <span>Darksmog.ai</span>
      ...
      function appendMessage(text, sender = "bot") {
        ...
        div.innerHTML = `
          ...
          <div class="desibot-meta">${sender === "user" ? "✔✔" : ""} ${formatTime(new Date())}</div>
        `;
    ...
    `;
          ...
          <div class="desibot-meta">${msg.message === "user" ? "✔✔" : ""} ${formatTime(new Date(msg.timestamp))}</div>
        `;
        ...
        fetch(`${backendUrl}/api/chat/history`, {
          ...
          .then(data => {
            if (Array.isArray(data)) {
              ...
              if (data.length) {
                setTimeout(() => appendMessage(`👋 Welcome back, ${userName}!`, "bot"), 400);
              }
            }
          })
          .catch(() => appendMessage("⚠️ Couldn't load chat history.", "bot"));
      }
      ...
      function handleInput() {
        ...
        if (step === 0) {
          if (/^\d{10}$/.test(text)) {
            ...
            appendMessage("⚠️ Please enter a valid 10-digit mobile number to continue.", "bot");
            input.value = "";
          }
        } else if (step === 1) {
          userName = text;
          ...
          fetch(`${backendUrl}/api/chat/history`, {
            ...
            .then(data => {
              if (Array.isArray(data) && data.length > 0) {
                const welcomeMsg = appendMessage(`Welcome back, ${userName}! 😊<br>Glad to see you again.`, "bot");

                storedHistory = data;
                const btn = document.createElement("div");
                btn.className = "desibot-load-history";
                btn.textContent = "📜 View previous conversation";
                btn.onclick = () => {
                ...
                appendMessage(`Hi ${userName}! 👋<br>I'm DesiBot, your assistant.<br>How can I help you today?`, "bot");
              }
            })

            .catch(() => appendMessage("⚠️ Could not load history."));
             input.value = "";
        }
        else if (step === 2) {
          ...
          fetch(`${backendUrl}/api/chat/send`, {
            ...
            .then(data => {
              typingDiv.remove();
              appendMessage(data.message || "✅ Message received.");
            })
            .catch(() => {
              typingDiv.remove();
      ...
      if (isFirstTime) {
        appendMessage("👋 Hi there! I'm DesiBot, your smart assistant. To get started, please enter your 10-digit mobile number.", "bot");
        isFirstTime = false;
      }
    };
  });
